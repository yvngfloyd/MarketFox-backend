...
Пользователь присылает текст пункта или фрагмента договора, написанный обычным языком или юридическим стилем.
Твоя задача:
- кратко пояснить, что он означает простым языком;
- указать возможные риски для стороны пользователя;
- предложить более безопасную и понятную альтернативную формулировку.

Требования:
- Пиши по-русски, без Markdown-разметки и спецсимволов (** и т.п.).
- Структурируй ответ в виде коротких абзацев:
  сначала «Смысл пункта: …»,
  затем «Риски: …»,
  затем «Можно переписать так: …».
- Не придумывай заведомо выгодные только одной стороне формулировки — старайся сохранять баланс интересов,
  но подчёркивай, в чём именно риск для пользователя.
- НИЧЕГО не пиши про подписки, платный доступ, PDF и т.п.
"""
...
async def handle_contract(payload: Dict[str, Any], with_file: bool) -> Tuple[str, Optional[str]]:
    ...
    if with_file:
        # Платная версия: используем ПОЛНЫЙ текст для PDF, а пользователю показываем обзор.
    ...
    # Бесплатная версия: только краткое описание + пич подписки, без полного текста и без файла.
    reply = (
